...
def bottleneck(inputs, depth, depth_bottleneck, stride, rate=1,
  ...
  with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
    ...
    residual = slim.conv2d(inputs, depth_bottleneck, [1, 1], stride=1, activation_fn=tf.nn.relu,
                           scope='conv1')
    ...
    # alphas = tf.get_variable('alpha', out.get_shape()[-1],
    #                              initializer=tf.constant_initializer(0.0),
    #                              dtype=tf.float32)
    # pos = tf.nn.relu(out)
    # neg = alphas * (out - abs(out)) * 0.5
    #
    # output = pos + neg
    output = tf.nn.relu(shortcut + residual)
