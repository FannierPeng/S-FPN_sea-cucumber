...
class FastRCNN(object):
    ...
    def fast_rcnn_net(self):
        ...
        with tf.variable_scope('fast_rcnn_net'):
            with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(self.weight_decay)):
                ...
                if self.use_dropout:
                    net = slim.dropout(net, keep_prob=0.5, is_training=self.is_training, scope='dropout',seed=1234)
    ...
    def fast_rcnn_find_positive_negative_samples(self, reference_boxes):
        ...
        with tf.variable_scope('fast_rcnn_find_positive_negative_samples'):
            gtboxes = tf.cast(
                tf.reshape(self.gtboxes_and_label[:, :-1], [-1, 4]), tf.float32)  # [M, 4]
            ...
            label = label * positives  # background is 0
    ...
    def fast_rcnn_loss(self):
        with tf.variable_scope('fast_rcnn_loss'):
            ...
            positive_proposals_in_img = draw_box_with_color(self.img_batch,
                                                            ...
                                                            text=
                                                            tf.shape(tf.where(tf.equal(minibatch_object_mask, 1.0)))[0])
            ...
            negative_proposals_in_img = draw_box_with_color(self.img_batch,
                                                            minibatch_reference_boxes * tf.expand_dims(negative_mask,
                                                                                                       1),
                                                            text=
                                                            tf.shape(tf.where(tf.equal(minibatch_object_mask, 0.0)))[0])
            ...
            with tf.variable_scope('fast_rcnn_classification_loss'):
                ...
                fast_rcnn_classification_loss = tf.multiply(tf.multiply(alpha_t, slim.losses.softmax_cross_entropy(logits=logits,
                                                            onehot_labels=onehot_labels)),tf.pow(1 - predictions_pt, new_gamma))
                ##FL
                # fast_rcnn_classification_loss = tf.multiply(tf.multiply(alpha_t, slim.losses.softmax_cross_entropy(logits=logits,
                #                                                    onehot_labels=onehot_labels)), tf.pow(1-predictions_pt, 2))

                #FL和PFL，不注释这句；CE注释这句
                fast_rcnn_classification_loss = tf.reduce_sum(fast_rcnn_classification_loss[:,0]+fast_rcnn_classification_loss[:,1])
