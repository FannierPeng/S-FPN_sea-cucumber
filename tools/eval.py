...
def eval_dict_convert(img_num,is_val):
...
def val(is_val):
    img_num = 259
    tf.set_random_seed(1234)
    eval_dict_convert(img_num,is_val)
    ...
    fr1.close()
    fr2.close()
    return np.array([Recall, Precision, mAP, F_measure],dtype=np.float32)
